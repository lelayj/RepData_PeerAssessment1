...
**dayTotal has `r nrow(dayTotal)` days instead of the `r length(unique(df$date))` days of October and November. Which means that the days that do not appear in dayTotal are days for each of which all of the corresponding 288 elements of df\$steps are NAs.**
...
**The days that do not appear in dayTotal are `r length(omittedDays)` in total and are the following:  `r omittedDays`.**  
...
**The mean and median total number of steps taken per day are `r sf(mean1)` and `r sf(median1)`, respectively.**  

**HISTOGRAM OF THE TOTAL NUMBER OF STEPS TAKEN EACH DAY, ALSO SHOWING THE MEAN AND THE MEDIAN:**
```{r plot1 histogram, fig.width= 10,fig.height= 10,echo=TRUE}
library(ggplot2)
...
**There is only `r length(maxiMeanIndices)` interval that contains the maximum number of steps (`r sf(maxiMean)`), on average across all the days in the data set, this one starting at `r h`.**    

**TIME SERIES PLOT OF THE AVERAGE NUMBER OF STEPS TAKEN, averaged across all days, versus the 5-minute intervals:**
```{r plot2 time series, fig.width= 10,fig.height= 10,echo=TRUE}
...
**The total number of rows with NAs is equal to `r length(NAStepsIndices)`.**
...
**The data frame new\_df contains `r length(which(is.na(new_df$steps)))` NAs: this data frame is the original data frame df in which each NA element of df\$steps has been replaced with the mean number of steps of the corresponding 5-minute interval.**  
...
**The new mean total number of steps taken per day is `r sf(mean2)` and the new median is `r sf(median2)` as well.**  

**HISTOGRAM OF THE TOTAL NUMBER OF STEPS TAKEN EACH DAY FOR THE NEW DATA SET, ALSO SHOWING THE MEAN AND THE MEDIAN:**
```{r plot3 new data set histogram, fig.width=10, fig.height=10, echo=TRUE}
library(ggplot2)
...
```{r plot4 means comparison, fig.width= 10,fig.height= 10, echo=TRUE}
...
- As we have just seen, the total number of steps in new\_df for each of the 8 days mentioned above, sum of mean\_i (i = 1,..., 288), is  equal to mean1 = `r sf(mean1)` (= mean2). 

- the median of dayTotal\$steps, **median1, is equal to `r median1`**.  
...
- the greatest index for which  a number equal to median1 can be found in sort(dayTotal\$steps) is `r max(median1Indices)`. 
...
**Only the 8 days containing  NAs have seen changes: their estimates  did not exist in dayTotal, since these days were removed. Each of them shows now a total daily number of steps equal to mean1 = `r sf(mean1)` (= mean2), whereas the estimates of the other 53 days remain the same.**
...
**MAKING A PANEL PLOT:**
```{r plot5 panel, fig.width= 10,fig.height= 10,echo=TRUE}
